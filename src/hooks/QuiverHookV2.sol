...
import {IQuiverHookV2PoolExtension} from "./interfaces/IQuiverHookV2PoolExtension.sol";
...
import {IQuiverHookV2} from "./interfaces/IQuiverHookV2.sol";
...
abstract contract QuiverHookV2 is BaseHook, IQuiverHookV2 {
    ...
    IQuiverPoolExtensionAllowlist public immutable poolExtensionAllowlist;
    address public immutable weth;

    mapping(PoolId => bool) public quiverIsToken0;
    ...
    constructor(
        ...
        factory = _factory;
        poolExtensionAllowlist = IQuiverPoolExtensionAllowlist(_poolExtensionAllowlist);
        weth = _weth;
    }
    ...
    function _initializePoolExtensionData(
        ...
        if (_poolExtension != address(0)) {
            ...
            IQuiverHookV2PoolExtension(_poolExtension).initializePreLockerSetup(
                poolKey, quiverIsToken0[poolKey.toId()], poolExtensionData
            );
        ...
        onlyFactory
    {
        ...
        if (poolExtension[poolKey.toId()] != address(0)) {
            IQuiverHookV2PoolExtension(poolExtension[poolKey.toId()]).initializePostLockerSetup(
                poolKey, locker[poolKey.toId()], quiverIsToken0[poolKey.toId()]
            );
    ...
    function _runMevModule(
        ...
        if (mevModuleOperational(poolKey.toId())) {
            ...
            bool disableMevModule = IQuiverMevModule(mevModule[poolKey.toId()]).beforeSwap(
                poolKey, swapParams, quiverIsToken0[poolKey.toId()], poolSwapData.mevModuleSwapData
            );
    ...
    function _runPoolExtensionHelper(
        ...
        IQuiverHookV2PoolExtension(poolExtension[poolKey.toId()]).afterSwap(
            poolKey, swapParams, delta, quiverIsToken0[poolKey.toId()], swapData
        );
    }
    ...
    function _afterSwap(
        ...
        if (isExactInput && swappingForQuiver || !isExactInput && !swappingForQuiver) {
            if (quiverIsToken0[poolKey.toId()]) {
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IQuiverHook).interfaceId
            || interfaceId == type(IQuiverHookV2).interfaceId;
    }
